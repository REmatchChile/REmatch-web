...
class Viewer extends Component {
  ...
  createEditor(type) {
    return CodeMirror(
      document.getElementById(`${type}Editor-${this.props.idx}`),
      {
      ...
      <Paper className='viewerPaper' elevation={3}>
        ...
        <div className='queryContainer'>
          {this.state.rematch ? (
            <div
              className={`queryEditor ${
                this.state.rematch && this.state.regex ? "centered" : null
              }`}
          ...
          {!(this.state.rematch && this.state.regex) ? (
            <Button
              disabled={
                this.state.rematchMatches.length !== 0 ||
                this.state.regexMatches.length !== 0
              }
              className='button'
              ...
              onClick={
                this.state.rematch
                  ? this.runREmatch.bind(this)
                  : this.runRegex.bind(this)
              }>
              ...
              {this.state.regexMatches.map((span, idxSpan) => (
                <div key={idxSpan} className='matchesRow disabled'>
                  ...
                  {span.map((capture, idxCapture) => (
                    <div
                      key={idxCapture}
                      className={`cm-m${idxCapture} matchesItem`}>
                      {capture}
                    </div>
                  ))}
                </div>
